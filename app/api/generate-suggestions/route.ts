...
function generateProjectSuggestions(resumeData: any, scholarData: any): ProjectSuggestion[] {
  ...
  if (techSkills.hasDataScience || researchFocus.isDataFocused) {
    suggestions.push(...generateDataScienceProjects(techSkills, academicLevel, researchFocus, skills, researchInterests))
  }
  ...
  suggestions.push(...generateInterdisciplinaryProjects(techSkills, academicLevel, researchFocus, skills, researchInterests))
  ...
  console.log('Top 3 suggestions:', finalSuggestions.slice(0, 3).map(s => ({ title: s.title, score: s.matchScore })))
...
function analyzeSkills(skills: string[]) {
  ...
  return {
    hasML: skillsLower.some(s => 
      s.includes('machine learning') || s.includes('tensorflow') || s.includes('pytorch') || 
      s.includes('scikit-learn') || s.includes('deep learning') || s.includes('neural')
    ),
    hasWeb: skillsLower.some(s => 
      s.includes('javascript') || s.includes('react') || s.includes('vue') || s.includes('angular') ||
      s.includes('html') || s.includes('css') || s.includes('web') || s.includes('frontend') || s.includes('backend')
    ),
    hasDataScience: skillsLower.some(s => 
      s.includes('python') || s.includes('r') || s.includes('pandas') || s.includes('numpy') ||
      s.includes('data analysis') || s.includes('statistics') || s.includes('visualization')
    ),
    hasMobile: skillsLower.some(s => 
      s.includes('react native') || s.includes('flutter') || s.includes('swift') || s.includes('kotlin') || s.includes('mobile')
    ),
    hasCloud: skillsLower.some(s => 
      s.includes('aws') || s.includes('azure') || s.includes('gcp') || s.includes('docker') || s.includes('kubernetes')
    ),
    hasDatabase: skillsLower.some(s => 
      s.includes('sql') || s.includes('mongodb') || s.includes('postgresql') || s.includes('database')
    ),
...
function determineAcademicLevel(education: any[], experience: any[], publicationsCount: number, citations: number) {
  const hasPhD = education.some(e => e.degree?.toLowerCase().includes('phd') || e.degree?.toLowerCase().includes('doctorate'))
...
function analyzeResearchFocus(interests: string[], publications: any[]) {
  ...
  return {
    isMLFocused: allText.includes('machine learning') || allText.includes('artificial intelligence') || 
                 allText.includes('deep learning') || allText.includes('neural network'),
...
function generateMLProjects(techSkills: any, academicLevel: string, researchFocus: any, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  ...
  const hasDeepLearning = techSkills.programmingLanguages.includes('Python') && userSkills.some(s => s.toLowerCase().includes('deep learning'))
  ...
  if (researchFocus.isBioFocused) {
    projects.push({
      ...
      description: "Create an intelligent system that assists in biomedical research by analyzing medical literature, predicting drug interactions, and identifying potential research directions.",
  ...
  if (researchFocus.isSecurityFocused) {
    projects.push({
      ...
      description: "Develop machine learning models for real-time cybersecurity threat detection and response, incorporating privacy-preserving techniques.",
  ...
  if (projects.length === 0) {
    projects.push({
      ...
      description: "Build an AI system that analyzes academic papers, researcher profiles, and citation networks to recommend relevant research directions and collaborations.",
...
function generateWebProjects(techSkills: any, academicLevel: string, researchFocus: any, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  const projects: ProjectSuggestion[] = []
  const primaryInterests = researchFocus.primaryDomains || []
...
function generateDataScienceProjects(techSkills: any, academicLevel: string, researchFocus: any, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  const projects: ProjectSuggestion[] = []
  const primaryInterests = researchFocus.primaryDomains || []
  const mainDomain = primaryInterests[0] || 'research'
  ...
  if (researchFocus.isBioFocused) {
    projects.push({
      ...
      description: "Apply advanced data mining techniques to genomic and clinical datasets to discover potential biomarkers and predict treatment outcomes.",
    ...
    projects.push({
      ...
      description: "Develop data science solutions for analyzing security logs, network traffic, and threat intelligence to identify patterns and predict cyber attacks.",
    ...
    projects.push({
      ...
      description: "Analyze social media data and user behavior patterns to understand trends, sentiment, and social dynamics in your research domain.",
...
function generateResearchProjects(scholarData: any, techSkills: any, academicLevel: string, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  const projects: ProjectSuggestion[] = []
  const publications = scholarData.publications || []
  ...
  if (publications.length > 0) {
    ...
    const pubTopics = recentPubs.map((p: any) => p.title || '').join(' ')
  ...
  if (hasHighCitations && publications.length > 5) {
    projects.push({
      ...
      researchAreas: ["Natural Language Processing", "Information Retrieval", primaryInterests[0] || "Research Methods"],
      difficulty: "Advanced",
  ...
  if (scholarData.hIndex && scholarData.hIndex > 5) {
    projects.push({
      ...
      skillsRequired: ["Graph Analytics", "Machine Learning", "Network Science", "Academic APIs", "Recommendation Systems"],
...
function generateInterdisciplinaryProjects(techSkills: any, academicLevel: string, researchFocus: any, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  const projects: ProjectSuggestion[] = []
  const primaryInterests = researchFocus.primaryDomains || []
  ...
  if (researchFocus.isHCIFocused || userInterests.some(i => i.toLowerCase().includes('interface') || i.toLowerCase().includes('user'))) {
  ...
  if (projects.length === 0) {
    projects.push({
      ...
      description: "Create a system that discovers connections and patterns across multiple research domains, facilitating interdisciplinary insights and collaboration opportunities.",
...
function generateOpenSourceProjects(techSkills: any, academicLevel: string, userSkills: string[], userInterests: string[]): ProjectSuggestion[] {
  const projects: ProjectSuggestion[] = []
  ...
  projects.push({
    ...
    skillsRequired: ["Programming", "Software Engineering", "Documentation", "Testing", "Git", "Open Source Development"],
  ...
  if (academicLevel === 'Advanced') {
    projects.push({
      ...
      skillsRequired: ["Educational Design", "Content Creation", "Web Development", "Video Production", "Community Building"],
...
function calculateMatchScore(
  ...
  userInterests.forEach(interest => {
    ...
    if (titleLower.includes(interestLower) || descriptionLower.includes(interestLower) || categoryLower.includes(interestLower)) {
